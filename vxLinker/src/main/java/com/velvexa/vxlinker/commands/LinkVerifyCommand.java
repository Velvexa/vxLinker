...
public class LinkVerifyCommand extends ListenerAdapter {
        ...
        if (rewardManager != null) {
            ...
            if (logManager != null) {
                logManager.logReward(
                        player.getUniqueId(),
                        "first-link",
                        MessageUtil.format("reward-log.saved", Map.of("type", "first-link", "player", playerName))
                );
            }
        }
    ...
    private void applyVerifiedRole(SlashCommandInteractionEvent event) {
        ...
        guild.addRoleToMember(finalMember, role).queue(
                success -> plugin.getLogger().info(MessageUtil.format("discord.role-success", Map.of("player", finalMember.getEffectiveName()))),
                error -> plugin.getLogger().warning(MessageUtil.format("discord.role-fail", Map.of("error", error.getMessage())))
        );
    }
    ...
    private void sendEmbed(SlashCommandInteractionEvent event, String title, String desc, Color color) {
        EmbedBuilder embed = new EmbedBuilder()
                ...
                .setDescription(desc)
                .setColor(color)
                .setFooter(MessageUtil.get("decorations.prefix").replace("&", "§"))
